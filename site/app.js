(function () {
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      var l = p.lessons[i];
      var lessonPath = l.path || (l.url ? ((l.url.match(/(phases\/[^/]+\/[^/]+)\/?$/) || [])[1] || '') : '');
      ...
      if ((l.status === 'complete' || userComplete) && lessonPath) {
        var lessonHref = window.addCourseParam ? window.addCourseParam('lesson.html?path=' + lessonPath) : 'lesson.html?path=' + lessonPath;
        actionHtml = '<a href="' + lessonHref + '" class="modal-lesson-read">' + (userComplete ? 'Review' : 'Read') + '</a>';
      }
      var toggleHtml = '';
