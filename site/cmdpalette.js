...
(function () {
    ...
    if (typeof PHASES !== 'undefined' && Array.isArray(PHASES)) {
      for (var i = 0; i < PHASES.length; i++) {
        ...
        for (var j = 0; j < phase.lessons.length; j++) {
          ...
          var lessonPath = lesson.path || (lesson.url ? ((lesson.url.match(/(phases\/[^/?#]+\/[^/?#]+)/) || [])[1] || '') : '');
  ...
  function renderResults(results) {
    ...
    if (!query) {
      var _hint = 'Type to search lessons and glossary terms';
      list.innerHTML =
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var _cp = window.addCourseParam || function(u) { return u; };
      if (r.kind === 'lesson') {
        // Prefer the in-site reader; fall back to GitHub URL
        dest = r.lessonPath
          ? _cp('lesson.html?path=' + encodeURIComponent(r.lessonPath))
          : r.url;
        chip = 'Phase ' + String(r.phaseId).padStart(2, '0');
      } else if (r.kind === 'artifact') {
        // Jump to the lesson that produced this artifact
        var ghBase = window.ACTIVE_COURSE
          ? 'https://github.com/' + window.ACTIVE_COURSE.githubRepo + '/tree/' + window.ACTIVE_COURSE.githubBranch + '/'
          : 'https://github.com/Linzo99/anything-from-scratch/tree/main/';
        dest = r.lessonPath
          ? _cp('lesson.html?path=' + encodeURIComponent(r.lessonPath))
          : (ghBase + r.file);
        ...
        dest      = _cp('glossary.html?q=' + encodeURIComponent(r.name));
        chip      = 'Glossary';
        chipClass += ' cp-item-chip--alt';
      }
